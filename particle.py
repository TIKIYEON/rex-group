...
def getWeights(particleList):
    weightArray = []
    for i in range(len(particleList)):
        weightArray[i] = particleList[i].getWeight()
    return weightArray

def sumWeights(particleList):
    sum = 0.0
    for i in range(len(particleList)):
        sum = sum + particleList[i].getWeight()
    return sum
...
## only gonna use one of the following uncertainty functions
